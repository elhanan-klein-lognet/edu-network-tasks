...
create table public.task_statuses (
  ...
  name            text not null,
  display_order   int not null default 0,
  -- Admin-configurable, not a hardcoded status name: marks which status(es)
  -- count as "done" for project progress (section 6) and the dashboard's
  -- overdue/completed counts (section 7). More than one status can be
  -- marked completed (e.g. both "הושלם" and "בוטל").
