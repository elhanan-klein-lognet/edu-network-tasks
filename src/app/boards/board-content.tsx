...
import { BoardKanban } from "./[id]/board-kanban";

const CAN_CREATE_TASKS = ["super_admin", "network_admin", "institution_manager"];
const CAN_LINK_PROJECTS = ["super_admin", "network_admin"];

/**
 * Fetches and renders one board's Kanban content. Shared by boards/[id]/page.tsx
 * (a specific board picked from the list) and boards/page.tsx (the default —
 * first board — shown when landing on plain /boards), so "default-select the
 * first board" is just "render this component with the first board's id"
 * instead of a redirect + a second request round trip.
 */
export async function BoardContent({ boardId }: { boardId: string }) {
